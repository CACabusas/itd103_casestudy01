...
function AdminUpdateStudent() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get(`http://localhost:3001/admin/getstudent/${studentId}`);
    ...
    const handleUpdate = (e) => {
        e.preventDefault();
        axios.put(`http://localhost:3001/admin/${teacherId}/updatestudent/${studentId}`, { name, sex, section })
            .then(res => {
                console.log(res);
                navigate(`/admin/view`);
            })
